...
namespace DuckGame.R6S
{
    ...
    public class GamemodeScripter : Thing, IDrawToDifferentLayers
    {
        ...
        public List<RenderTarget2D> _replayFrames = new List<RenderTarget2D>();
        public int replayFrame;
        public int replaySize = 300; //5 seconds
        ...
        public float preparationPhase = 15f;
        ...
        public List<float> phaseTime = new List<float>(); // 0 - Selection, 1 - Loading, 2 - Preparation, 3 - Active, 4 - Postplant, 5 - End
        ...
        List<MaterialThing> possibleThings = new List<MaterialThing>();
        List<MaterialThing> observationPOVs = new List<MaterialThing>();
        public GamemodeScripter() : base()
        {
        ...
        public override void Initialize()
        {
            ...
            phaseTime.Add(preparationPhase);
        ...
        public override void Update()
        {
            ...
            if (time <= 0 && currentPhase > 0 && currentPhase < 6)
            {
                if(currentPhase == 2)
                {
                    spawned = false;
                ...
                if (currentPhase == 2)
                {
                    ...
                    {
                        if (op.team == "Def" /*&& op.local*/ && op.roundStartPhrase != "")
                        {
                            Level.Add(new SoundSource(position.x, position.y, 320, op.roundStartPhrase, "J"));
                        }
                    }
                }
                if (currentPhase == 3)
                {
                    foreach(Attacker att in Level.current.things[typeof(Attacker)])
                    {
                        att.effects.Add(new SpawnArmor());
                    }
                    GiveDefuser();
                }
                if(currentPhase == 4)
                {
                    ...
                    if (team == 1)
                    {
                        SFX.Play(GetPath("SFX/Announcer/ActionStart.wav"), 1);
                    }
                }
                if(currentPhase == 5)
                {
                    if (team == 0)
                    {
                        SFX.Play(GetPath("SFX/Announcer/DefuserPlantedD.wav"), 1);
                    }
                    if (team == 1)
                    {
                        SFX.Play(GetPath("SFX/Announcer/DefuserPlanted.wav"), 1);
                    }
                }
            }
            else
        ...
        public virtual void PreparationPhase()
        {
            ...
            //DevConsole.Log(Convert.ToString(t), Color.White);
            if (confirmed >= t)
            {
                //DevConsole.Log("SKIP SKIP SKIP", Color.White);
                if (time > 3)
            ...
            if (selected != null)
            {
                ...
                {
                    spawned = true;
                    selected.oper.position = GetSpawnPosition();
                    selected.oper._sleeping = false;
                }
                else
                {
                    if (selected != null)
                    {
                        foreach (Operators oper in Level.current.things[typeof(Operators)])
                        {
                            if (oper != selected.oper)
                            {
                                ...
                                if (oper.team == selected.oper.team)
                                {
                                    oper.c = new Vec3(200, 200, 255).ToColor();
                                }
                                else
                                {
                                    oper.c = new Vec3(255, 150, 150).ToColor();
                                }
                            }
                        }
                    }
        ...
        public virtual void ActionPhase()
        {
            ...
            foreach (Operators op in Level.current.things[typeof(Operators)])
            {
                if (op.team == "Att" && Level.current is GameLevel)
                {
                    ...
                    op.lockedTakeOut[3] = 10;
                    op.lockedTakeOut[4] = 10;
                    if (op.inventory.Count > 2)
                    {
                        (op.inventory[1] as GunDev).shotFrames = 0.05f;
                        (op.inventory[2] as GunDev).shotFrames = 0.05f;
                    }
                }
            }
        ...
        public virtual void AfterplantPhase()
        {
            UpdateVision();
            ...
            if (!isTH)
            {
                ...
                if (!endRound)
                {
                    if (def == 0 && ((!defused && planted) || !planted))
                    {
                        TeamWon("Att");
                        endRound = true;
                        currentPhase++;
        ...
        public virtual void TakeScreenshot()
        {
            /*if (Level.current.camera != null)
            {
                ...
                Vec2 renderTargetSize = new Vec2(1920f, 1080f);
                ...
                if(_replayFrames.Count >= replaySize)
                {
                    _replayFrames.RemoveAt(0);
                }
                _replayFrames.Add(_captureTarget);
            }*/
        }
        ...
        public virtual void UpdateVision()
        {
            if (selected != null)
            {
                ...
                if (selected.oper.head != null && selected.oper.local)
                {
                    ...
                    possibleThings.Clear();

                    foreach (MaterialThing m in Level.current.things[typeof(MaterialThing)])
                    {
                        if (m is Operators || m is ObservationThing)
                        {
                            if (!observationPOVs.Contains(m))
                            {
                                observationPOVs.Add(m);
                            }
                        }

                        if (m is Operators || m is OperatorHead || m is Holdable_plus)
                        {
                            if (!(m is LandFire))
                            {
                                possibleThings.Add(m);
                            }
                        }
                        if (isTH && (m is Terrorist || m is TerroristHead))
                        {
                            possibleThings.Add(m);
                        }
                    }
                    ...
                    foreach (MaterialThing m in possibleThings)
                    {
                        float exposure = ExposedAmountToAllPOV(m);
                        if(exposure > 0)
                        {
                            exposure = 1;
                        }
                        m.alpha = exposure;
                        if (selected != null && selected.oper != null)
                        {
                            if (m is ObservationThing && (m as ObservationThing).team == selected.oper.team && !(m as ObservationThing).broken && !(m as ObservationThing).jammed)
                            {
                                exposure = 1;
                            }
                            if(m is Operators && (m as Operators) == selected.oper)
                            {
                                exposure = 1;
                            }
                        }
        ...
        public virtual float ExposedAmountToAllPOV(MaterialThing thing)
        {
            float exposure = 0;
            foreach(MaterialThing m in observationPOVs)
            {
                if(m is Operators)
                {
                    Operators oper = m as Operators;
                    if (!oper.isDead && selected != null && oper == selected.oper)
                    {
                        if(Level.CheckLine<Block>(thing.position, oper.position + oper.headPosition) == null 
                            && (Level.CheckLine<SmokeGR>(thing.position, oper.position + oper.headPosition) == null ||
                            selected.oper.seeTSmokeFrames > 0))
                        {
                            exposure += 0.5f;
                        }
                        if (Level.CheckLine<Block>(thing.bottomRight, oper.position + oper.headPosition) == null 
                            && (Level.CheckLine<SmokeGR>(thing.bottomRight, oper.position + oper.headPosition) == null ||
                            selected.oper.seeTSmokeFrames > 0))
                        {
                            exposure += 0.35f;
                        }
                        if (Level.CheckLine<Block>(thing.topLeft, oper.position + oper.headPosition) == null
                            && (Level.CheckLine<SmokeGR>(thing.topLeft, oper.position + oper.headPosition) == null ||
                            selected.oper.seeTSmokeFrames > 0))
                        {
                            exposure += 0.35f;
                        }
                        if (Level.CheckLine<Block>(thing.bottomLeft, oper.position + oper.headPosition) == null
                            && (Level.CheckLine<SmokeGR>(thing.bottomLeft, oper.position + oper.headPosition) == null ||
                            selected.oper.seeTSmokeFrames > 0))
                        {
                            exposure += 0.35f;
                        }
                        if (Level.CheckLine<Block>(thing.topRight, oper.position + oper.headPosition) == null
                            && (Level.CheckLine<SmokeGR>(thing.topRight, oper.position + oper.headPosition) == null ||
                            selected.oper.seeTSmokeFrames > 0))
                        {
                            exposure += 0.35f;
                        }
                ...
                if(m is ObservationThing)
                {
                    ObservationThing obs = m as ObservationThing;
                    if (obs.observing && !obs.jammed && !obs.broken && selected != null && obs.team == selected.oper.team)
                    {
                        float cameraImpact = exposure;
                        if (Level.CheckLine<Block>(thing.position, obs.position) == null && Level.CheckLine<SmokeGR>(thing.position, obs.position) == null)
                        {
                            exposure += 0.35f;
                        }
                        if (Level.CheckLine<Block>(thing.bottomRight, obs.position) == null && Level.CheckLine<SmokeGR>(thing.bottomRight, obs.position) == null)
                        {
                            exposure += 0.35f;
                        }
                        if (Level.CheckLine<Block>(thing.topRight, obs.position) == null && Level.CheckLine<SmokeGR>(thing.topRight, obs.position) == null)
                        {
                            exposure += 0.35f;
                        }
                        if (Level.CheckLine<Block>(thing.bottomLeft, obs.position) == null && Level.CheckLine<SmokeGR>(thing.bottomLeft, obs.position) == null)
                        {
                            exposure += 0.35f;
                        }
                        if (Level.CheckLine<Block>(thing.topLeft, obs.position) == null && Level.CheckLine<SmokeGR>(thing.topLeft, obs.position) == null)
                        {
                            exposure += 0.35f;
                        }

                        cameraImpact = exposure - cameraImpact;

                        if (thing is Operators)
                        {
                            Operators op = thing as Operators;
                            if ((op.MainDevice is InvisibilityForDrones || op.MainDevice is HEL) && (op.MainDevice as ExPoD).enabled)
                            {
                                exposure -= cameraImpact;
                            }
                        }
                    }
                }
        ...
        public virtual void OperatorsSelect(int[] ban)
        {
            ...
            if (team == 0)
            {
                ...
                for (int i = 0; i < PlayerStats.totalDefenders; i++)
                {
                    ...
                    {
                        Level.Add(new OperatorSelection(camPos.x + gapX + step * l, camPos.y + gapY + step * m) 
                        { operatorID = op.oper.operatorID, Scale = new Vec2(Scale, Scale) });
                        k++;
                    }
                }
        ...
        public virtual void SiteSelect()
        {
            SiteSelector selector = Level.current.things[typeof(SiteSelector)].First() as SiteSelector;
            if (selector != null)
            {
                int sites = 0;
                foreach (string site in selector.FirstSite)
                {                    
                    Level.Add(new ManualSiteSelection(camPos.x + 70, camPos.y + 60 + 20 * sites) { SiteID = sites, SiteName = site, siteSelector = selector, gamemode = this });
                    sites++;
                }
            }
        }
        public virtual void SiteSelectRemove()
        {
            foreach (ManualSiteSelection selection in Level.current.things[typeof(ManualSiteSelection)])
            {
                Level.Remove(selection);
            }
        }
        public virtual void SelectPlantRoom(int index)
        {
            foreach (SiteSelector s in Level.current.things[typeof(SiteSelector)])
            {
                if(index < s.FirstSite.Count)
                {
                    s.selectedSite = index;
                }
            }
        }
        public virtual void SetPlantRoom()
        {
            string point = "";
            string point2 = "";
            foreach (SiteSelector s in Level.current.things[typeof(SiteSelector)])
            {
                if (s.FirstSite.Count > 0)
                {
                    ...
                    {
                        s.selectedSite = s.FirstSite.Count - 1;
                    }

                    //THIS IS NOT SYNCED
                    //MUST BE SELECTED BY PLAYERS
                    if (Network.isActive)
                    {
                        if (Network.isServer)
                        {
                            point = s.FirstSite[s.selectedSite];
                            DuckNetwork.SendToEveryone(new NMSiteSelected(point));
                        }
                    }
                    else
                    {
                        point = s.FirstSite[s.selectedSite];
                        point2 = s.SecondSite[s.selectedSite];

                        DevConsole.Log(point + " + " + point2);
                    }
                }
            }
            foreach (SiteSelector s in Level.current.things[typeof(SiteSelector)])
            ...
            //RemoveSpawnPositoins();
        }
        public void RemoveSpawnPositoins()
        {
            string point1 = "";
            string point2 = "";
            ...
            foreach (SiteSelector s in Level.current.things[typeof(SiteSelector)])
            {
                if (s.FirstSite.Count > 0)
                {
                    if (s.selectedSite < s.FirstSite.Count)
                    {
                        point1 = s.FirstSite[s.selectedSite];
                        point2 = s.SecondSite[s.selectedSite];
                    }
                }
            }
            ...
            foreach(Spawn s in Level.current.things[typeof(Spawn)])
            {
                if(s.team == "Def" && (s.location != point1 || s.location != point2))
                {
                    Level.Remove(s);
                }
            }
        ...
        public void GiveDefuser()
        {
            if (selected != null)
            {
                foreach (OPEQ opeq in selectedOperators)
                {
                    if (opeq.oper != null && opeq.oper.team == "Att" && opeq.duckOwner != null)
                    {
                        opeq.oper.HasDefuser = true;
                        DuckNetwork.SendToEveryone(new NMPickUpDefuser(opeq.oper.position, opeq.duckOwner.netProfileIndex));
                        return;
                    }
                }
            }
        ...
        public virtual void DrawLoadingPhase()
        {
            ...
            if (selectedOperators.Count > 0)
            {
                foreach (OPEQ opeq in selectedOperators)
                {
                    ...
                    {
                        if ((opeq.oper.team == "Def" && team == 0) || (opeq.oper.team == "Att" && team == 1))
                        {
                            ...
                            _banner.depth = 0.1f;
                            Graphics.Draw(_banner, Level.current.camera.position.x + (17 + step * playr) * Unit, Level.current.camera.position.y + 39 * Unit);


                            //user profile avatar
                            if (playr == 0 && localDuck != null)
                            {
                                ...
                                //Graphics.Draw(avatar.texture, Level.current.camera.position + new Vec2((25f + step * playr) * Unit, 116f * Unit), null, Color.White, 0, new Vec2(), new Vec2(0.68f, 0.68f), SpriteEffects.None, 1 + 0.1f);
                            }
                            ...
                            */
                            ...
                            Graphics.DrawStringOutline(opeq.name, Level.current.camera.position + new Vec2(56 * Unit - opeq.name.Length / 2 * 8 * Unit + step * playr * Unit, 160 * Unit), Color.White, Color.Black, 0.6f, null, 0.5f);
                            if (opeq.duckOwner != null)
                            {
                                //DevConsole.Log(opeq.duckOwner.profile.name);
                                Graphics.DrawStringOutline(opeq.duckOwner.profile.name, Level.current.camera.position + new Vec2(24 * Unit/* - opeq.oper.name.Length / 2 * 8 * Unit*/ + step * playr * Unit, 47 * Unit), Color.White, Color.Black, 0.6f, null, 0.5f);
                            ...
                            icon.CenterOrigin();
                            Graphics.Draw(icon, Level.current.camera.position.x + (47 + step * playr) * Unit, Level.current.camera.position.y + 132 * Unit, 0.6f);
        ...
        public virtual void DrawOperatorsSelect() //Everytime, except changemap-phase
        {
            ...
            if (currentPhase > 0)
            {
                ...
                if (time > 0)
                {
                    ...
                    {
                        int timer = (int)time;
                        int f = (int)(time * 10) % 10;
                        string text = Convert.ToString(timer) + "." + Convert.ToString(f);
                        Graphics.DrawStringOutline(text, Level.current.camera.position - new Vec2(160f + 2 * text.Length * UIScale, 180 - 2 * UIScale) * Unit + new Vec2(320, 180) * Unit, Color.LightYellow, Color.Black, 0.5f, null, 0.5f * Unit);
                    }
                }
                ...
                if (selectedOperators.Count > 0)
                {
                    foreach (OPEQ opeq in selectedOperators)
                    {
                        ...
                        if (opeq.oper.team == "Def")
                        {
                            ...
                            if (team == 0)
                            {
                                Graphics.DrawLine(new Vec2(pos.x + (startOffset - HealthWideness * 0.5f) * Unit * UIScale, pos.y + 6 * Unit * UIScale), 
                                    new Vec2(pos.x + (startOffset - HealthWideness * 0.5f + HealthWideness) * Unit * UIScale, pos.y + 6 * Unit * UIScale), Color.Black, 1f * Unit * UIScale, 0.45f);
                                Graphics.DrawLine(new Vec2(pos.x + (startOffset - HealthWideness * 0.5f) * Unit * UIScale, pos.y + 6 * Unit * UIScale), 
                                    new Vec2(pos.x + (startOffset - HealthWideness * 0.5f + HealthWideness * (opeq.oper.Health / 100f)) * Unit * UIScale, pos.y + 6 * Unit * UIScale), Color.White, 1f * Unit * UIScale, 0.65f);
        ...
        public override void Draw()
        {
            ...
            if(Level.current is Editor)
            {
                Graphics.DrawString(Convert.ToString(Level.current.things.Count) + " / 10000", new Vec2(10, 170), Color.White);
            }
        ...
        public void OnDrawLayer(Layer pLayer)
        {
            if (pLayer == Layer.Foreground)
            {
                /*if (currentPhase == 5)
                {
                    if (replayFrame < _replayFrames.Count)
                    {
                        Graphics.Draw(_replayFrames[replayFrame], Level.current.camera.position - Level.current.camera.size * 0.5f, 
                            null, Color.White, 0, Level.current.camera.size * 0.5f, new Vec2(320 / (1920f), 180 / (1080f)), SpriteEffects.None, 1);
                        //Graphics.Draw(_replayFrames[replayFrame], Level.current.camera.position.x, Level.current.camera.position.y, 1f, 1f, 1f);
                        replayFrame++;
                    }
                }*/
            }
